...
export function useLatestAnomaliesPerVehicle(): Record<string, Anomaly> {
  ...
  const latestPerVehicle: Record<string, Anomaly> = {}
  for (const anomaly of filtered) {
    if (
      !latestPerVehicle[anomaly.vehicle_id] ||
...
export function useAnomalyTimeline(): Anomaly[] {
  const { data = [] } = useAnomalies()
  const dismissedAnomalies = useUIStore((state) => state.dismissedAnomalies)
